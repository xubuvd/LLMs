...
def step1(text):
    ...
    # g25 clean_parenthese
    cleaned_content = cleaner.clean_parenthese(cleaned_content)
    # new policy
    ...
    # g20 TooLongSentence
    # cleaned_content = cleaner.is_chinese_long_strings_without_punctuation(cleaned_content)
...
def val_step(js_dict):
    count = 0
    for key in KEYS:count += val_pro(js_dict[key])
    if count <= VAL_FILETR:
        return "VAL_FILETR", 0, js_dict['content']
    else: return "", 1, js_dict['content']
...
def make_clean(items):
    ...
    with open(fpath,"r",encoding="utf-8") as f:
        ...
        for line_no,line in tqdm(enumerate(datas),total=len(datas)):
            ...
            # clean_status, clean_status, content = val_step(js_dict, VAL_FILETR)
