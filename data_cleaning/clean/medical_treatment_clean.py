...
WeChatCleaner = CleanHeadTailsFromContent("/localdisk/datacleaner/utils/wechat_ads_phrase.txt")
...
def step1(text):
    ...
    # deal with ![text](url)
    cleaned_content = cleaner.clean_special_url(cleaned_content)
    ...
    is_common_zhLessThan20 = cleaner.common_zhLessThan20(text)
    if is_common_zhLessThan20:
        return "is_common_zhLessThan60", 0, cleaned_content

    is_weak_table_page = cleaner.is_WeakTablePage(text)
...
def step2(text):
    ...
    is_sentive,key_words = BadChecker.is_spam_text(text,
        thresh_hold=3,
        black_dataType=["badword","gumble","ads"] #remove sex and dirty
    )
    if is_sentive:
