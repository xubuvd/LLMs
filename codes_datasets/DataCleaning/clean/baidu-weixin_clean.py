...
from utils.check_political_toxic import CheckToxicWords
...
WeChatCleaner = CleanHeadTailsFromContent("./utils/wechat_ads_phrase.txt")
ToxicChecker = CheckToxicWords()
def step1(text):
...
def step2(text):
    ...
    toxicity,badtext = ToxicChecker.is_toxic_text(text, thresh_hold=1)
    if toxicity:
        return "is_toxic_text:{}".format(badtext), 0, text
    return "",1,text
