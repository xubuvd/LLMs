// compile: gcc -o word2phrase word2phrase.c -lm
...
long long vocab_max_size = 1000000, vocab_size = 0;
...
void TrainModel() {
  ...
  while (1) {
    ...
    if (oov) score = 0;
        ...
        score = (pab - min_count)/((real)pa*(real)pb/doc_count + sqrt(pab)*ln_pq);
    }
    if (score > threshold) {
      //fprintf(fo, "%s", word);
      pb = 0;
      fprintf(fo,"%d\t%s\t%.3f\n", line_no,bigram_word,score);
    }
