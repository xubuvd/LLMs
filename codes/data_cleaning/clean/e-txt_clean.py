...
WeChatCleaner = CleanHeadTailsFromContent("./utils/ebook_lowwords.txt",thresh_hold=1)

def step1(text):

    if len(text) < _TEXT_LONG_REQUIRED_:
        return "TooShortSentence", 0, text
    cleaned_content = text[4096:-1024]
    cleaned_content = cleaner.clean_head(cleaned_content)
...
def parse_args():
    ...
    parser.add_argument('--dest_path',
                        type=str,
                        default="/localdist/llm/clean_data/cn-e-txt/",
                        help='Directory containing trained actor model')
